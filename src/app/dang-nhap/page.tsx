...
function LoginForm() {
  ...
  // Xử lý đăng nhập Google cho sinh viên
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error) {
        setErrorMsg("Chức năng Google OAuth chưa cấu hình. Vui lòng đăng nhập bằng MSSV bên dưới.");
        setLoading(false);
      }
    } catch {
      setErrorMsg("Không thể kết nối Google. Vui lòng đăng nhập bằng MSSV.");
      setLoading(false);
    }
  };
  ...
  const handleAccountLogin = async (e: React.FormEvent) => {
    ...
    // 1. ĐĂNG NHẬP DÀNH CHO ADMIN & BCH CHI ĐOÀN
    if (loginRole === "admin") {
      // 1.1. Tài khoản Admin tối cao duy nhất
      ...
      // 1.2. Kiểm tra tài khoản BCH Chi đoàn từ Supabase
      ...
      setLoading(false);
      setErrorMsg("Tài khoản hoặc mật khẩu Quản trị viên / Cán bộ Chi đoàn không chính xác.");
      return;
    }

    // 2. ĐĂNG NHẬP DÀNH CHO SINH VIÊN
    try {
      const { data: studentData, error } = await supabase
        .from("students")
        .select("*")
        .eq("mssv", cleanUser.toUpperCase())
        .single();

      if (error || !studentData) {
        setLoading(false);
        setErrorMsg("Mã số sinh viên chưa tồn tại trong danh sách sinh viên.");
        return;
      }
      ...
      if (cleanPass !== expectedPassword && cleanPass !== cleanUser.slice(-3)) {
        setLoading(false);
        setErrorMsg("Mật khẩu không đúng. Mật khẩu mặc định là 3 số cuối của MSSV.");
        return;
      }
      ...
      setLoading(false);
      setErrorMsg("Lỗi máy chủ cơ sở dữ liệu: " + err.message);
    }
  };
      ...
      {loginRole === "student" && (
        <div className="space-y-3">
          <button
            ...
            disabled={loading}
            className="w-full bg-white hover:bg-slate-50 text-slate-700 font-bold py-2.5 px-4 rounded-xl border border-slate-300 transition text-xs flex items-center justify-center gap-3 shadow-xs"
          >
            ...
            <span>Đăng nhập bằng Google sinh viên</span>
          </button>
      ...
      <form onSubmit={handleAccountLogin} className="space-y-3.5">
        <div>
          <label className="block text-xs font-bold text-slate-700 mb-1">
            {loginRole === "admin" ? "Tên tài khoản (Admin / BCH Chi đoàn) *" : "Mã số sinh viên (MSSV) *"}
          </label>
          <input
            type="text"
            required
        ...
        <div>
          ...
          <input
            ...
            placeholder={loginRole === "admin" ? "Mật khẩu quản trị / cán bộ" : "Mật khẩu (Mặc định: 3 số cuối MSSV)"}
            className="w-full border border-slate-300 rounded-xl px-4 py-2.5 text-xs outline-none focus:border-[#EE6425]"
          />
        </div>
        ...
        >
          {loading ? "Đang xác thực..." : loginRole === "admin" ? "Đăng nhập Quản Trị" : "Đăng nhập Sinh viên"}
        </button>
      </form>
    </div>
  );
      ...
      <div className="max-w-md w-full bg-white rounded-3xl p-8 shadow-xl border border-slate-200">
        <div className="text-center mb-6">
          ...
          <p className="text-xs text-slate-500 mt-1">
            Tuổi trẻ Khoa Kỹ thuật Cơ khí - CTUtT
          </p>
        </div>
